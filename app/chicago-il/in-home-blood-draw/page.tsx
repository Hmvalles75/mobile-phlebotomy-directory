...
export default function ChicagoInHomeBloodDraw() {
  ...
  useEffect(() => {
    async function fetchProviders() {
      try {
        const params = new URLSearchParams({ city: 'Chicago', state: 'IL', grouped: 'true' })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <CityBreadcrumb citySlug="chicago-il" cityShort="Chicago" variant="in-home-blood-draw" />
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 text-white">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-4">In-Home Blood Draw Services in Chicago, IL</h1>
          <p className="text-xl text-primary-100">Professional at-home blood draw services in Chicago — request same-day or next-day availability.</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-12 max-w-4xl space-y-8">
        <InlineLeadForm city="Chicago" state="IL" />

        {!loading && providers.length > 0 && (
          <div className="bg-white rounded-lg shadow p-8">
            <h2 className="text-2xl font-bold mb-6">Mobile Phlebotomists Serving Chicago</h2>
            <div className="space-y-6">
              {providers.slice(0, 5).map(provider => (
                <div key={provider.id} className="border-b border-gray-200 pb-6 last:border-0">
                  ...
                  <ProviderActions provider={provider} currentLocation="Chicago, IL" variant="compact" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-8">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <Link key="Naperville" href={"/us/illinois/naperville"}
              ...
              <span className="font-medium text-gray-900">Naperville, IL</span>
            </Link>
            <Link key="Evanston" href={"/us/illinois/evanston"}
              className="block bg-gray-50 rounded-lg p-4 hover:bg-primary-50 border border-gray-200 hover:border-primary-300 transition-all">
              <span className="font-medium text-gray-900">Evanston, IL</span>
            </Link>
            <Link key="Oak Park" href={"/us/illinois/oak-park"}
              className="block bg-gray-50 rounded-lg p-4 hover:bg-primary-50 border border-gray-200 hover:border-primary-300 transition-all">
              <span className="font-medium text-gray-900">Oak Park, IL</span>
            </Link>
            <Link key="Schaumburg" href={"/us/illinois/schaumburg"}
              className="block bg-gray-50 rounded-lg p-4 hover:bg-primary-50 border border-gray-200 hover:border-primary-300 transition-all">
              <span className="font-medium text-gray-900">Schaumburg, IL</span>
            </Link>
            <Link key="Joliet" href={"/us/illinois/joliet"}
              className="block bg-gray-50 rounded-lg p-4 hover:bg-primary-50 border border-gray-200 hover:border-primary-300 transition-all">
              <span className="font-medium text-gray-900">Joliet, IL</span>
            </Link>
            <Link key="Aurora" href={"/us/illinois/aurora"}
              className="block bg-gray-50 rounded-lg p-4 hover:bg-primary-50 border border-gray-200 hover:border-primary-300 transition-all">
              <span className="font-medium text-gray-900">Aurora, IL</span>
            </Link>
          </div>
        </div>
        ...
        <CityFAQ cityName="Chicago, IL" cityShort="Chicago" variant="in-home-blood-draw" />

        <CityIntentVariantLinks citySlug="chicago-il" cityName="Chicago, IL" current="in-home-blood-draw" />
      </div>
