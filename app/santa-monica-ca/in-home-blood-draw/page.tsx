...
export default function SantaMonicaInHomeBloodDraw() {
  ...
  useEffect(() => {
    async function fetchProviders() {
      try {
        const params = new URLSearchParams({ city: 'Santa Monica', state: 'CA', grouped: 'true' })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <CityBreadcrumb citySlug="santa-monica-ca" cityShort="Santa Monica" variant="in-home-blood-draw" />
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 text-white">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-4">In-Home Blood Draw Services in Santa Monica, CA</h1>
          <p className="text-xl text-primary-100">Professional in-home blood draw services in Santa Monica — request same-day or next-day availability.</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-12 max-w-4xl space-y-8">
        <InlineLeadForm city="Santa Monica" state="CA" />

        {!loading && providers.length > 0 && (
          <div className="bg-white rounded-lg shadow p-8">
            <h2 className="text-2xl font-bold mb-6">Mobile Phlebotomists Serving Santa Monica</h2>
            <div className="space-y-6">
              {providers.slice(0, 5).map(provider => (
                <div key={provider.id} className="border-b border-gray-200 pb-6 last:border-0">
                  ...
                  <ProviderActions provider={provider} currentLocation="Santa Monica, CA" variant="compact" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-8">
          <h2 className="text-2xl font-bold mb-4">About In-Home Blood Draw Services in Santa Monica</h2>
        ...
        <CityFAQ cityName="Santa Monica, CA" cityShort="Santa Monica" variant="in-home-blood-draw" />

        <CityIntentVariantLinks citySlug="santa-monica-ca" cityName="Santa Monica, CA" current="in-home-blood-draw" />
      </div>
      <LeadFormModal isOpen={leadFormOpen} onClose={() => setLeadFormOpen(false)} defaultCity="Santa Monica" defaultState="CA" defaultZip="" />
    </div>
  )
}
