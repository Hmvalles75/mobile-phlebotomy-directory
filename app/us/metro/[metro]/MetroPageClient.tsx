...
interface MetroPageClientProps {
  ...
  /** Fetched on the server so the listing exists in the HTML Googlebot sees. */
  initialProviders: Provider[]
  initialGrouped: GroupedProviders | null
  /**
   * Providers whose service radius actually covers this metro. The description
   * counts these, not the full list — the same rule the city pages follow.
   * Regional providers are still listed and badged, but describing them as
   * being "in {City}" is the overstatement that rewrite removed.
   */
  localCount: number
}

export default function MetroPageClient({ params, initialProviders, initialGrouped, localCount }: MetroPageClientProps) {
  const [searchQuery, setSearchQuery] = useState('')
  const [selectedServices, setSelectedServices] = useState<string[]>([])
  // Seeded from the server render. This component no longer fetches: it exists
  // for the search and service filters, which genuinely need to be interactive.
  const providers = initialProviders
  const groupedResults = initialGrouped
  // Data arrives with the first render, so there is never a loading state.
  const loading = false
  ...
  const providerListSchema = generateProviderListSchema(
    ...
    `Find ${localCount} certified mobile phlebotomy providers in ${metro.city}, ${metro.state}. Professional at-home blood draws starting at ${metro.localInfo?.avgCost || '$60-120'}. ${metro.localInfo?.typicalWaitTime || '24-48 hour'} appointments.`,
