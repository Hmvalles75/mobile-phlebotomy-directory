...
export default function BrooklynInHomeBloodDraw() {
  ...
  useEffect(() => {
    async function fetchProviders() {
      try {
        const params = new URLSearchParams({ city: 'Brooklyn', state: 'NY', grouped: 'true' })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <CityBreadcrumb citySlug="brooklyn-ny" cityShort="Brooklyn" variant="in-home-blood-draw" />
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 text-white">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-4">In-Home Blood Draw Services in Brooklyn, NY</h1>
          <p className="text-xl text-primary-100">Professional in-home blood draw services in Brooklyn — request same-day or next-day availability.</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-12 max-w-4xl space-y-8">
        <InlineLeadForm city="Brooklyn" state="NY" />

        {!loading && providers.length > 0 && (
          <div className="bg-white rounded-lg shadow p-8">
            <h2 className="text-2xl font-bold mb-6">Mobile Phlebotomists Serving Brooklyn</h2>
            <div className="space-y-6">
              {providers.slice(0, 5).map(provider => (
                <div key={provider.id} className="border-b border-gray-200 pb-6 last:border-0">
                  ...
                  <ProviderActions provider={provider} currentLocation="Brooklyn, NY" variant="compact" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-8">
          <h2 className="text-2xl font-bold mb-4">About In-Home Blood Draw Services in Brooklyn</h2>
          <p className="text-gray-700 leading-relaxed">Licensed phlebotomists come to your Brooklyn-area location for convenient in-home blood draw services. Services include routine lab work, drug testing, wellness panels, and specialty collections. Same-day and next-day appointments are typically available throughout the Brooklyn area.</p>
        </div>

        <CityFAQ cityName="Brooklyn, NY" cityShort="Brooklyn" variant="in-home-blood-draw" />

        <CityIntentVariantLinks citySlug="brooklyn-ny" cityName="Brooklyn, NY" current="in-home-blood-draw" />
      </div>
