...
export default function BurbankBloodDrawAtHome() {
  ...
  useEffect(() => {
    async function fetchProviders() {
      try {
        const params = new URLSearchParams({ city: 'Burbank', state: 'CA', grouped: 'true' })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <CityBreadcrumb citySlug="burbank-ca" cityShort="Burbank" variant="blood-draw-at-home" />
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 text-white">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-4">Blood Draw at Home in Burbank, CA</h1>
          <p className="text-xl text-primary-100">Professional blood draw at home in Burbank — request same-day or next-day availability.</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-12 max-w-4xl space-y-8">
        <InlineLeadForm city="Burbank" state="CA" />

        {!loading && providers.length > 0 && (
          <div className="bg-white rounded-lg shadow p-8">
            <h2 className="text-2xl font-bold mb-6">Mobile Phlebotomists Serving Burbank</h2>
            <div className="space-y-6">
              {providers.slice(0, 5).map(provider => (
                <div key={provider.id} className="border-b border-gray-200 pb-6 last:border-0">
                  ...
                  <ProviderActions provider={provider} currentLocation="Burbank, CA" variant="compact" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-8">
          <h2 className="text-2xl font-bold mb-4">About Blood Draw at Home in Burbank</h2>
        ...
        <CityFAQ cityName="Burbank, CA" cityShort="Burbank" variant="blood-draw-at-home" />

        <CityIntentVariantLinks citySlug="burbank-ca" cityName="Burbank, CA" current="blood-draw-at-home" />
      </div>
