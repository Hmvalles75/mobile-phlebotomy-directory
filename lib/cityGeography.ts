...
/**
 * Pure geography for city listings — no database, no server-only, no caching.
 *
 * Split out of lib/providers-city.ts so the page path and the static-page
 * generator (scripts/upgrade-city-page.ts) share ONE bucketing rule. They
 * previously each had their own, which is how /metros and the 18 generated
 * overrides ended up publishing numbers the pages themselves disagreed with.
 * A generator that computes its own count is a third rule waiting to drift.
 */
...
export function bucketProvidersForCity(
  providers: any[],
  cityName: string,
  stateAbbr: string,
): GroupedCityProviders {
