...
const LONGFORM = CITY_LONGFORM[${JSON.stringify(`${stateSlug(state)}/${slug}`)}]
...
export default async function ${name.replace(/[^a-zA-Z]/g, '')}MobilePhlebotomyPage() {
  const [seoProviders, nearbyCities] = await Promise.all([
    getProvidersForCity(${JSON.stringify(slug)}, ${JSON.stringify(state.toUpperCase())}),
    getNearbyCities(${JSON.stringify(slug)}, ${JSON.stringify(state.toUpperCase())}, 8),
  ])
    ...
    <>
      ...
      {LONGFORM && (
        <section className="container mx-auto px-4 pt-8 max-w-4xl">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Mobile Phlebotomy in ${name}: A Local Guide</h2>
      ...
      <div className="bg-gray-50">
        <div className="container mx-auto px-4 pb-12">
          <ProvidersInCity providers={seoProviders} cityName={${JSON.stringify(name)}} stateAbbr={${JSON.stringify(state.toUpperCase())}} />
          <NearbyCities
            cities={nearbyCities}
            sourceCitySlug={${JSON.stringify(slug)}}
            sourceCityName={${JSON.stringify(name)}}
            sourceStateName={${JSON.stringify(stateName)}}
          />
        </div>
      </div>
    </>
