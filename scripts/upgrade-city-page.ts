...
function generatePageContent(data: CityData): string {
  ...
  const localBusinessSchema = JSON.stringify({
    ...
    url: `https://mobilephlebotomy.org/us/${state.toLowerCase()}/${slug}`,
    areaServed: {
      '@type': 'City',
      name,
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'United States', item: 'https://mobilephlebotomy.org/us' },
      {
        ...
        position: 3,
        name: stateName,
        item: `https://mobilephlebotomy.org/us/${state.toLowerCase()}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
        name,
        item: `https://mobilephlebotomy.org/us/${state.toLowerCase()}/${slug}`,
      },
    ],
  })
...
//   npx tsx scripts/upgrade-city-page.ts --city=${slug} --state=${state.toLowerCase()}
//
...
export const metadata: Metadata = {
  ...
  keywords: ${JSON.stringify(
    `mobile phlebotomy ${name.toLowerCase()}, mobile phlebotomist ${name.toLowerCase()}, ${name.toLowerCase()} mobile blood draw, at home blood draw ${name.toLowerCase()}, mobile lab ${name.toLowerCase()}, ${name.toLowerCase()} ${state.toLowerCase()} phlebotomy, ${stateName.toLowerCase()} mobile phlebotomy`
  )},
  alternates: {
    canonical: ${JSON.stringify(`https://mobilephlebotomy.org/us/${state.toLowerCase()}/${slug}`)},
  },
  openGraph: {
    title: ${JSON.stringify(title)},
    description: ${JSON.stringify(description)},
    url: ${JSON.stringify(`https://mobilephlebotomy.org/us/${state.toLowerCase()}/${slug}`)},
    type: 'website',
  },
}
...
export default function ${name.replace(/[^a-zA-Z]/g, '')}MobilePhlebotomyPage() {
  return (
    <>
      ...
      <CityPage params={{ state: ${JSON.stringify(state.toLowerCase())}, city: ${JSON.stringify(slug)} }} />
    </>
  )
}
`
...
async function main() {
  ...
  const outDir = path.join(process.cwd(), 'app', 'us', stateAbbr.toLowerCase(), slug)
