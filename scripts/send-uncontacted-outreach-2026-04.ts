...
const CAMPAIGN = 'uncontacted-outreach-2026-04'
...
async function main() {
  ...
  // Find all uncontacted providers with real email addresses AND a known state
  // (state required so we can personalize the lead-count hook)
  ...
  for (const p of providers) {
    ...
    // Skip providers with malformed scraped email data (e.g., image paths, URL fragments)
    if (!EMAIL_REGEX.test(email)) {
      console.log('SKIP (malformed email): ' + p.name + ' — ' + p.email)
      skippedJunk++
      continue
    }
